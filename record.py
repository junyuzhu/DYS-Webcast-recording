...
dysurl = "http://3954.liveplay.myqcloud.com/live/3954_549922684.flv"
...
def record():
    # 调用ffmpeg进行直播录像
    print("----正在录像中----")
    os.system("sudo ffmpeg -i " + dysurl + " -c copy -f flv "
              "/home/pi/dir/videorecord/$(date +%Y%m%d%H%M%S).flv")
    print("----本次录像完毕----")
...
def main():
    ...
    while True:
        ...
        if current_time >= 19 or current_time < 3:  # 如果当前时间在设定的直播时间内，调用request_link函数
            try:
                request_link()
